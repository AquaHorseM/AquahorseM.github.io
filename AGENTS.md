...
## Project Notes
- Static GitHub Pages homepage: primary editable files are `index.html` and `stylesheet.css`; keep `data/CV_QinweiMa.pdf` unchanged unless explicitly requested.
- Homepage content should avoid fabricated dates, authors, links, acceptances, and metrics. For the Stanford experience, do not invent an end month.
- Publication sections currently use selected publications with thumbnails only where useful and a compact text-only list for other publications.
